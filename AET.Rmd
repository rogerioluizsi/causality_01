...
ggplot(dt
       ,aes(p_logistic, fill = as.factor(Wmod), color = as.factor(Wmod))) +
...
```
Inverse-propensity score weighting in Binary outcome - Here the propensity score serves a single-dimensional variable that summarizes how observables affect the treatment probability.
...
```{r}
#plot variables balance before matching
library(ggplot2)
theme_set(
  theme_classic() + 
...
ggplot(df_bin, aes(x = RENDA_PERCAPITA, fill= as.factor(W))) + geom_boxplot()
